...
import { z } from "zod";
import { handlerSubmitOrder } from "./action";
...
export function OrderCreate({
  ...
  async function handleProcessOrder(data: z.infer<typeof formSchema>) {
    const result = await handlerSubmitOrder(data);

    if (!result) {
      return // show toast error
    }
  ...
  return (
    ...
    >
      <form
        className="space-y-8"
        onSubmit={form.handleSubmit(handleProcessOrder)}
      >
          ...
          <div>
            ...
            >
              ...
              <CardFooter className="flex flex-row items-center border-t bg-muted/50 px-6 py-3">
                <Button type="submit" className="w-full"><MdOutlinePayment className="mr-2 w-5 h-5" /> Payment</Button>
              </CardFooter>
            </Card>
          </div>
        </section>
